...
TEMP_CONSOLE_TOKEN_SECRET = secrets.token_hex(32)
TEMP_CONSOLE_TOKEN_EXPIRY_MINUTES = 5
...
class ProxmoxService:
    ...
    def _call_proxmox_api(self, api_call_func, *args, **kwargs):
        try:
            ...
            logger.warning(f"Proxmox API认证失败，尝试重新连接: {auth_err}")
            self._connect()
            return api_call_func(*args, **kwargs)
        except Exception as e:
            ...
            if "authentication failed" in error_message or \
               ...
               "ticket" in error_message:
                logger.warning(f"Proxmox API认证/会话错误，尝试重新连接: {e}")
                self._connect()
                return api_call_func(*args, **kwargs)
            else:
                logger.error(f"Proxmox API调用时发生未知错误: {e}")
                raise
        ...
        try:
            ...
            for key, value in config.items():
                if key.startswith("net") and isinstance(value, str):
                    ...
                    for part in parts:
                        if '=' in part:
                            k, v_part = part.split('=', 1)
                            net_details[k.strip()] = v_part.strip()
                        ...
                        if "ip" in net_details:
                            ...
                            if ip_config.lower() == "dhcp":
                                ...
                                if current_status.get("status") != "running":
                                    logger.info(f"容器 {node}/{vmid} 未运行，无法通过agent获取DHCP IP。")
                                    return None
                                try:
                                    ...
                                    logger.warning(f"未能从agent获取容器 {node}/{vmid} 的DHCP IP。Agent响应: {interfaces_info}")
                                except Exception as agent_e:
                                    logger.warning(f"尝试通过agent获取容器 {node}/{vmid} IP时出错: {agent_e}")
                                return None
                            else:
                                ip_with_cidr = ip_config.split('/')[0]
                                return str(ipaddress.ip_address(ip_with_cidr))
            logger.warning(f"未能找到容器 {node}/{vmid} 上接口 {interface_name} 的IP配置。")
            return None
        except Exception as e:
            logger.error(f"获取容器 {node}/{vmid} IP地址时发生异常: {e}")
            return None
    ...
    def generate_console_session_token(self, node: str, vmid: str, user_id: Any) -> str:
        payload = {
            "exp": datetime.utcnow() + timedelta(minutes=TEMP_CONSOLE_TOKEN_EXPIRY_MINUTES),
            "iat": datetime.utcnow(),
            "nbf": datetime.utcnow(),
            "node": node,
            "vmid": vmid,
            "user_id": str(user_id), 
            "sub": "lxc_console_session"
        }
        token = jwt.encode(payload, TEMP_CONSOLE_TOKEN_SECRET, algorithm="HS256")
        return token

    def verify_console_session_token(self, token: str) -> bool:
        try:
            jwt.decode(token, TEMP_CONSOLE_TOKEN_SECRET, algorithms=["HS256"])
            return True
        except jwt.ExpiredSignatureError:
            logger.info("控制台会话令牌已过期。")
            return False
        except jwt.InvalidTokenError as e:
            logger.warning(f"无效的控制台会话令牌: {e}")
            return False
